...
mcp: AI向けツール公開 / 引数受け取り / Hono APIへの橋渡し
api: 業務ロジック / バリデーション / DB保存
db: 永続化 / 制約 / RLS

MCP サーバー側に業務ロジックやDBロジックを寄せすぎないこと。
...
表示用テキストを後から解析して JSON 化するのではなく、保存時に save_curriculum の引数として構造化データを確定する
...
save_curriculum 自体は、DB保存や業務ロジックの本体を持たない。
...
ChatGPT がカリキュラムを提示する
ユーザーが「保存して」と承認する
save_curriculum が呼ばれる
MCP サーバーが POST /curricula を叩く（Bearer で JWT を転送）
Hono 側で以下を行う
認証ユーザー取得
入力検証
...
slug の一意性は API 側でサフィックス付与により解決するため、slug 競合を理由とした 409 は返さない。

実装順序
apps/mcp の作成
MCP サーバーの最小起動確認
save_curriculum ツール定義
apps/api に POST /curricula 実装
save_curriculum → POST /curricula 接続
curriculum + topics 一括保存の確認
ChatGPT 上で保存フローの疎通確認
実装方針
最初は save_curriculum 1本だけ に絞る
MCP サーバーは薄く保つ
DBロジックは API 側に寄せる
