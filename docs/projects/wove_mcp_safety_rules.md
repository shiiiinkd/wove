...
### [API] curricula.ts: 入力バリデーション未実装（最優先）

- ファイル: `apps/api/src/routes/curricula.ts`
- 問題: `req.json()` の内容を直接使用しており、型・構造の検証がない
- 対処: `SaveCurriculumSchema` を定義し、`safeParse` による検証を追加する

### [API] curricula.ts: エラーハンドリング未統一

- ファイル: `apps/api/src/routes/curricula.ts`
- 問題: バリデーションエラー時のレスポンスが統一されていない
- 対処: `safeParse` 失敗時に 400 を返す処理を統一する

### [API] curricula.ts: topics 配列の厳密検証不足

- ファイル: `apps/api/src/routes/curricula.ts`
- 問題: topics の件数・orderIndex 重複・必須項目の保証が不十分
- 対処:
  - `.min(1)` を追加する
  - `.max(20)` 程度の上限を追加する
  - `orderIndex` 重複チェックを `refine` で実装する

### [API] curricula.ts: 文字数制限未定義

- ファイル: `apps/api/src/routes/curricula.ts`
- 問題: title / description の長さ制限が未定義
- 対処:
  - `title.max(100)` を設定する
  - `description.max(1000)` を設定する

### [API] topics.ts: PATCH 入力のバリデーション未実装

- ファイル: `apps/api/src/routes/topics.ts`
- 問題: title / description の空文字・長さ制限が未検証
- 対処: Zodスキーマを導入し、更新時の入力検証を追加する

### [API] summaries.ts: content のバリデーション未実装

- ファイル: `apps/api/src/routes/summaries.ts`
- 問題: content の長さ制限がない
- 対処: `.max(10000)` 程度の制約を追加する

### [API] 全体: バリデーション方針未統一

- 問題:
  - 一部APIで未検証
  - parse / safeParse の使い分けが未整理
- 対処:
  - 全APIで `safeParse` ベースに統一する
  - バリデーションエラーは 400 を返すルールを統一する
